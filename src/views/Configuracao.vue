<template>
  <body>
    <div class="card-body" v-if="autorizacao === '[ROLE_ADMIN]'">
      ...
      <table id="usuario" class="table table-striped" v-if="showUsuarios">
        <thead>
          <br />
          <br />
          <tr
        ...
        >
          <tr v-for="usuario in usuarios" :key="usuario.id">
            ...
            <td>{{ usuario.email }}</td>
            <td
              v-for="autorizacao in usuario.autorizacoes"
              v-bind:key="autorizacao.id"
            >
              {{
...
export default {
  ...
  methods: {
    ...
    cadastrar() {
      if (this.nome.length <= 3) {
        ...
        const usuarioDTO = {
          ...
          senha: this.senha,
          autorizacao:
            this.autorizacao2 == "ADMINISTRADOR" ? "ROLE_ADMIN" : "ROLE_USER",
        };
        axios
          .post("/usuario/novo", usuarioDTO)
    ...
    atualizar() {
      ...
      axios
        .put("/usuario/put/" + this.id, usuarioDTO)
