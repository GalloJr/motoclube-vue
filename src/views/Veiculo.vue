<template>
  <body>
    ...
    >
      ...
      <table id="veiculo" class="table table-striped" v-if="showVeiculos">
        <thead>
          <br />
          <br />
          <tr
        ...
        >
          ...
          <div class="form-group col-md-12">
            ...
            <input
              ...
              autofocus
              v-model="placa"
              placeholder="Ex: Placa(WYK-0000 ou WYk-0A22)"
            />
          </div>
...
export default {
  ...
  methods: {
    ...
    buscarTodos() {
      axios
        .get("/veiculos/all", { headers: { Accept: "application/json" } })
        .then((res) => {
    ...
    buscarApelido(ap) {
      axios
        .get("/veiculos/ap/" + ap, { headers: { Accept: "application/json" } })
        .then((res) => {
          this.showVeiculos = true;
          console.log(res);
          this.veiculos = res.data;
    ...
    cadastrar() {
      if (this.placa.lengt > 8 || this.placa.lengt < 8) {
        ...
        alert("Marca muito grande");
      } else if (this.modelo.length > 20) {
        alert("Modelo muito grande");
        ...
        axios
          .post("/veiculos/create", membroDTO)
    ...
    atualizar() {
      ...
      axios
        .put("/veiculos/put/" + this.id, veiculoDTO)
